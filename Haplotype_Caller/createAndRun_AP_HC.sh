...
REFFILE=$1 # provided by scanning
FQLIST=$2 # provided by scanning
...
# Analysis Setup for HC
for i in "${BAMLIST[@]}" ; do
  OUTNAME=$(echo "$i" | cut -d. -f1)
  OUTNAME=$(echo "hc.${OUTNAME}.vcf")
  echo "gatk --java-options \"-Xmx16g\" HaplotypeCaller -R ${REFFILE} -I ${i} -O ${OUTNAME} &> ${i}.hc.single.log &" &>> stepHC_${TSTRING}
  echo "sleep 2" >> stepHC_${TSTRING}
done

echo "$PSTRING" >> stepHC_${TSTRING}
echo "touch stepHC_${TSTRING}_finished" >> stepHC_${TSTRING}
echo "completed setup creted file stepHC_${TSTRING}"

# Finally, run HC step
bash stepHC_${TSTRING}
while true ; do
